...
// handle all interactions with the TMDB API using Angular's dependency injection system

// Making a Request
// When a method is called from a component or another service,
//  HttpClient makes a GET request to the TMDB API with the appropriate endpoint,
//  including the API key and other parameters like page, language, region, or query.
//   The Observable returned by the HTTP client allows the
//  caller to asynchronously receive the data once the request is completed.
